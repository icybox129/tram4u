...
app.post("/", async (req, res) =>{   
    try {
        ...
        let firstDepartureTime, firstArrivalTime, secondDepartureTime, secondArrivalTime;
        if (stepsLength > 1) {
            ...
            res.render("result.ejs", { firstDepart: firstDepartureTime, firstArrival: firstArrivalTime, secondDepart: secondDepartureTime, secondArrival: secondArrivalTime });
        } else {
            firstDepartureTime = travelData.data.routes[0].legs[0].steps[0].transit_details.departure_time.text
            firstArrivalTime = travelData.data.routes[0].legs[0].steps[0].transit_details.arrival_time.text
            console.log("Rendering 2nd result.ejs");
            res.render("result.ejs", { firstDepart: firstDepartureTime, firstArrival: firstArrivalTime, secondDepart: null, secondArrival: null });
        }        
    } catch (error) {
        console.error(error);
